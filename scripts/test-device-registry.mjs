...
sql(`UPDATE auth.users SET deleted_at=now() WHERE id='${I.managerA}';`); expectReject('deleted manager denied',I.managerA,authorize(),/forbidden|not authorized|not active/i);
sql(`UPDATE auth.users SET deleted_at=NULL,banned_until=now()+interval '1 day' WHERE id='${I.managerA}';`); expectReject('banned manager denied',I.managerA,authorize(),/forbidden|not authorized|not active/i);
