...
const known = [record('rpc-client', "supabase.rpc('sale')"), record('edge-function', 'Deno.serve(handler)', 'supabase/functions/sale/index.ts')];
...
test('a substituted surface fails despite identical total counts', () => {
  assert.match(validate([record('rpc-client',"supabase.rpc('refund')"),known[1]],baseline).join('\n'), /rpc-client/);
});
test('a newly detected kind fails rather than being silently ignored', () => {
  assert.match(validate([...known,record('ipc-main','ipcMain.handle(...)')],baseline).join('\n'), /ipc-main/);
});
