...
const items = JSON.stringify([{
  ...
  quantity: "1.000",
  discount_fils: 0,
  // Deliberately false client hint. checkout_sale_v2 must resolve the server price (1.000 BHD / 1000 fils).
  unit_price_fils: 1,
  tax_rate: 99,
}]).replaceAll("'", "''");
const operationId = "delivery-authority-op-0001";
function deliveryCall({ address = "Amwaj Islands, Bahrain", feeFils = 250, op = operationId, branch = I.branchA } = {}) {
...
expectReject(
  "cross-tenant/branch delivery denied",
  I.cashierA,
  deliveryCall({ op: "delivery-authority-op-0002", branch: I.branchB }),
  /forbidden|branch/i,
);
expectReject(
