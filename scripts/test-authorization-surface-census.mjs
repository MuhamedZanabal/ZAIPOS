...
export function scanSource(path, body) {
  ...
  collect(/\.\s*from\s*\(\s*([^,\n)]+)/g, (match) => {
    const expression = normalizeExpression(match[1]);
    add('table-client', staticString(expression) ?? expression, match.index);
  });
  collect(/\bSUPABASE_SERVICE_ROLE_KEY\b/g, (match) => {
    add('privileged-credential', match[0], match.index);
  });
  ...
  collect(/\bGRANT\s+([^;]+?)\s*;/gi, (match) => add('sql-grant', match[1], match.index));
  collect(/\bREVOKE\s+([^;]+?)\s*;/gi, (match) => add('sql-revoke', match[1], match.index));
  collect(/\bCREATE\s+(?:CONSTRAINT\s+)?TRIGGER\s+("[^"]+"|[A-Za-z0-9_]+)/gi, (match) => {
    add('sql-trigger', match[1].replaceAll('"', ''), match.index);
  });
  collect(/\bcron\s*\.\s*schedule\s*\(\s*(['"])([^'"]+)\1/gi, (match) => {
    add('background-job', match[2], match.index);
  });
