...
5. A server actor/payload conflict remains unresolved, even when the local journal labels it `rejected`. Preserve the original request, retry its identity or seek manager reconciliation; do not start a replacement. Choose **Acknowledge session receipt** only after reviewing a matching recorded or cancelled server receipt. This clears the local receipt, not server evidence. A new operation receives a new identity. A pending request cannot be acknowledged away.
