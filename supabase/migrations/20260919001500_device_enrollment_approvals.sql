-- Stage 1 of trusted-device authority. An approval is NOT a device credential,
-- enrollment, or authorization to perform a financial operation. Existing
-- checkout and heartbeat entry points remain untrusted until later stages.
...
BEGIN
  IF _actor IS NULL OR _tenant_id IS NULL OR _branch_id IS NULL
     ...
     OR NOT public.has_branch_role(_actor, _tenant_id, _branch_id,
       ARRAY['owner', 'admin', 'manager']) THEN
  ...
  -- An already registered device must never be silently re-enrolled through
  -- a new approval. Revocation and rotation require separate audited flows.
  IF EXISTS (
    SELECT 1 FROM public.devices d
    WHERE d.tenant_id = _tenant_id AND d.device_uid = _device_uid
  ) THEN
  ...
  INSERT INTO public.audit_logs
    (tenant_id, branch_id, user_id, action, details)
  VALUES
    (_tenant_id, _branch_id, _actor, 'device.enrollment_approved',
     jsonb_build_object('approval_id', _approval_id, 'device_uid', btrim(_device_uid),
...
COMMENT ON FUNCTION public.approve_device_enrollment(uuid, uuid, text) IS
  'Manager-only, branch-scoped pending approval; does not enroll, credential, or authorize a device.';
