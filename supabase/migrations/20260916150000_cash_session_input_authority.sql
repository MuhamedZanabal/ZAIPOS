BEGIN;
-- Opening floats and blind counts are explicit business evidence. Never infer
-- missing counts as zero or round typed amounts before the server validates them.
CREATE OR REPLACE FUNCTION public.open_cash_session(
 _tenant_id uuid,_branch_id uuid,_opening_amount numeric DEFAULT NULL,_register_id uuid DEFAULT NULL
) RETURNS public.cash_sessions LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
DECLARE _session public.cash_sessions; _fils bigint;
BEGIN
 IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
 IF NOT public.has_branch_role(auth.uid(),_tenant_id,_branch_id,ARRAY['owner','admin','manager','cashier']::public.app_role[]) THEN RAISE EXCEPTION 'Forbidden'; END IF;
 ...
 IF _opening_amount IS NULL OR _opening_amount<0 OR _opening_amount<>round(_opening_amount,3) OR _opening_amount::text IN ('NaN','Infinity','-Infinity') THEN RAISE EXCEPTION 'Opening cash requires an explicit non-negative exact-fils amount'; END IF;
 _fils:=public.bhd_numeric_to_fils(_opening_amount);
 IF _register_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM public.cash_registers WHERE id=_register_id AND tenant_id=_tenant_id AND branch_id=_branch_id AND status='active') THEN RAISE EXCEPTION 'Cash register is not active in this branch'; END IF;
 IF EXISTS(SELECT 1 FROM public.cash_sessions WHERE tenant_id=_tenant_id AND branch_id=_branch_id AND status='open' AND register_id IS NOT DISTINCT FROM _register_id) THEN RAISE EXCEPTION 'This register already has an open session'; END IF;
 -- Existing unique partial indexes enforce one open session under concurrency.
 INSERT INTO public.cash_sessions(tenant_id,branch_id,register_id,user_id,opening_amount)
 VALUES(_tenant_id,_branch_id,_register_id,auth.uid(),public.fils_to_bhd_numeric(_fils)) RETURNING * INTO _session;
 INSERT INTO public.audit_logs(tenant_id,user_id,action,entity,entity_id,metadata)
 VALUES(_tenant_id,auth.uid(),'cash_session.opened','cash_sessions',_session.id,jsonb_build_object('branch_id',_branch_id,'register_id',_register_id,'opening_amount_fils',_fils));
 RETURN _session;
END; $$;
REVOKE ALL ON FUNCTION public.open_cash_session(uuid,uuid,numeric,uuid) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.open_cash_session(uuid,uuid,numeric,uuid) TO authenticated;
...
BEGIN
  ...
  IF _counted_amount IS NULL OR _counted_card IS NULL OR _counted_transfer IS NULL OR _counted_qr IS NULL THEN
    RAISE EXCEPTION 'Enter all four counted amounts explicitly, including zero';
  END IF;
