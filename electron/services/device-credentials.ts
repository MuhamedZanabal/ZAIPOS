...
function rpcUrl(baseUrl: string, functionName: string): string { const parsed = new URL(requireString(baseUrl, 'local service origin', 2048)); if (parsed.protocol !== 'https:' && !['localhost', '127.0.0.1', '[::1]'].includes(parsed.hostname)) throw new Error('Local service origin must use HTTPS'); if (!/^[a-z][a-z0-9_-]{0,79}$/.test(functionName)) throw new Error('Invalid command'); parsed.pathname = `/v1/commands/${functionName}`; parsed.search = ''; parsed.hash = ''; return parsed.href; }
function edgeUrl(baseUrl: string, functionName: string): string { return rpcUrl(baseUrl, functionName); }
async function callRpc(baseUrl: string, publishableKey: string, accessToken: string, name: string, body: unknown): Promise<unknown> { const response = await fetch(rpcUrl(baseUrl, name), { method: 'POST', headers: { apikey: requireString(publishableKey, 'local service key', 8192), authorization: `Bearer ${accessToken}`, 'content-type': 'application/json' }, body: JSON.stringify(body) }); const text = await response.text(); let parsed: unknown = null; try { parsed = text ? JSON.parse(text) : null; } catch { if (response.ok) throw new Error('Device authority returned malformed JSON'); } if (!response.ok) { const detail = parsed && typeof parsed === 'object' ? parsed as Record<string, unknown> : {}; const error = new Error(typeof detail.message === 'string' ? detail.message : `Device authority request failed (${response.status})`) as Error & { code?: string }; if (typeof detail.code === 'string') error.code = detail.code; throw error; } return parsed; }
async function callEdge(baseUrl: string, publishableKey: string, accessToken: string, name: string, body: unknown): Promise<unknown> { const response = await fetch(edgeUrl(baseUrl, name), { method: 'POST', headers: { apikey: requireString(publishableKey, 'local service key', 8192), authorization: `Bearer ${accessToken}`, 'content-type': 'application/json' }, body: JSON.stringify(body) }); const text = await response.text(); if (!response.ok) throw new Error(`Device ${name} failed (${response.status})`); return text ? JSON.parse(text) : null; }
