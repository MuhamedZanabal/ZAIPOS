...
async function initStore(): Promise<void> {
  ...
  store = new ElectronStore({ name: 'pos-settings', defaults: DEFAULT_SETTINGS });
...
function createWindow(settings: AppSettings): BrowserWindow {
  ...
  const win = new BrowserWindow({
    width: 1280, height: 800, minWidth: 1024, minHeight: 600, kiosk, frame: !kiosk, fullscreen: kiosk,
    titleBarStyle: kiosk ? 'hidden' : 'default', backgroundColor: '#0f0f0f', show: false,
    icon: path.join(__dirname, '../dist/pwa-512x512.png'),
    webPreferences: { preload: path.join(__dirname, 'preload.mjs'), contextIsolation: true, sandbox: true, nodeIntegration: false, webSecurity: true, allowRunningInsecureContent: false },
  });
  ...
  if (devUrl) {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol) || !['localhost', '127.0.0.1', '[::1]'].includes(parsed.hostname)) throw new Error('Desktop development server must use an explicit loopback origin');
    win.loadURL(devUrl);
...
function setupGlobalHandlers(): void {
  ...
  handleManagerIpc(IPC_HANDLERS.SAVE_SETTINGS, 'settings', async (_event, newSettings: Partial<AppSettings>) => {
    validateSettingsPatch(newSettings); if (!store) throw new Error('Settings are unavailable'); const current = getSettings(); const merged = { ...current, ...newSettings };
    if (newSettings.printer) merged.printer = { ...current.printer, ...newSettings.printer }; if (newSettings.barcode) merged.barcode = { ...current.barcode, ...newSettings.barcode };
    const validated = validateSettings(merged); store.set(validated); if (newSettings.barcode && mainWindow) await restartBarcodeScanner(validated.barcode, mainWindow);
  });
  handleManagerIpc(IPC_HANDLERS.SET_KIOSK, 'kiosk', async (_event, enabled: boolean) => {
    if (typeof enabled !== 'boolean') throw new Error('Invalid kiosk setting'); if (!mainWindow) return; store?.set('kiosk', enabled); mainWindow.setKiosk(enabled); mainWindow.setFullScreen(enabled); mainWindow.setMenuBarVisibility(!enabled); log('info', 'kiosk_mode_updated', { enabled });
  });
...
async function bootstrap(): Promise<void> {
  await initStore(); const settings = getSettings(); log('info', 'settings_loaded', { kiosk: settings.kiosk, printerType: settings.printer?.connectionType, barcodeMode: settings.barcode?.mode });
  setupGlobalHandlers(); setupPrinterHandlers(() => getSettings().printer); mainWindow = createWindow(settings);
  mainWindow.once('ready-to-show', async () => { if (mainWindow) await setupBarcodeScanner(settings.barcode, mainWindow); });
