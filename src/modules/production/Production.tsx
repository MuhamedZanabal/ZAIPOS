...
import { completeProductionOrderV2 } from "@/lib/inventory";

function StatusPill({ status }: { status: string }) {
  if (status === "completed") return <span className="pill pill-ok">Completed</span>;
  if (status === "in_progress" || status === "running") return <span className="pill pill-brand">In progress</span>;
  return <span className="pill pill-ghost">Draft</span>;
}
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { l: "In progress", v: String(stats.running), s: "active now" },
      ...
      <div className="flex items-center gap-2">
        {[
          ...
          { id: "in_progress", label: `In progress · ${stats.running}` },
            ...
            return (
              <div key={o.id} className={cn("g-prod-table-row", i % 2 === 1 && "g-prod-table-row-alt")}>
                <div className="flex items-center gap-3 min-w-0">
                  <div className="orb g-prod-orb"><Factory size={16} /></div>
    ...
    <DialogContent className="max-w-md">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <Input type="number" step="0.001" value={planned} onChange={(e) => setPlanned(e.target.value)} className="h-12 text-lg" />
        </div>
        <div className="space-y-1.5">
          <Label>Notes</Label>
  ...
  const submit = async () => {
    ...
    try {
      await completeProductionOrderV2({
        ...
        waste: Number(waste),
        clientMutationId: `production-complete-${order.id}`,
      });
      toast.success("Order completed · stock updated exactly once");
      setProduced(""); setWaste("0");
      onClose();
    } catch (error: any) {
      toast.error(error.message ?? "Could not complete production order");
    }
  };
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3">
          <div className="text-sm text-muted-foreground">
            {order?.products?.name} · planned {Number(order?.planned_quantity ?? 0).toFixed(2)}
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <Label>Waste</Label>
            <Input type="number" step="0.001" value={waste} onChange={(e) => setWaste(e.target.value)} />
          </div>
